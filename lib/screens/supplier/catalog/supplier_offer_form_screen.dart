...
class _SupplierOfferFormScreenState extends ConsumerState<SupplierOfferFormScreen> {
  ...
  bool _saving = false;
  int? _offerId; // edit mode if provided
  ...
  final _region = TextEditingController(text: 'Île-de-France');
  ...
  DateTime? _from;
  DateTime? _to;
  final _allergensCsv = TextEditingController(); // ids séparés par virgules (option)

  @override
  void dispose() {
    _name.dispose(); _desc.dispose(); _producer.dispose(); _region.dispose(); _unit.dispose();
    _price.dispose(); _minQty.dispose(); _stock.dispose(); _allergensCsv.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    final editor = ref.watch(supplierOfferEditorProvider);
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Row(children: [
                      Expanded(child: _DateField(label: 'Dispo du', value: _from, onPick: (d)=>setState(()=>_from=d))),
                      const SizedBox(width: 8),
                      Expanded(child: _DateField(label: 'au', value: _to, onPick: (d)=>setState(()=>_to=d))),
                    ]),
                    const SizedBox(height: 8),
                    TextFormField(
                      controller: _region,
  ...
  Future<void> _submit(BuildContext context, {required bool submit}) async {
    ...
    final payload = {
      ...
      if (_allergensCsv.text.trim().isNotEmpty)
        'allergens': _allergensCsv.text.trim().split(',').where((x)=>x.trim().isNotEmpty).map((x)=>int.parse(x.trim())).toList(),
    };
    ...
    try {
      if (_offerId == null) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Offre mise à jour')));
        Navigator.pop(context);
      }
    } catch (_) {
      if (mounted) {
...
class _DateField extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final txt = value == null ? '' : '${value!.year}-${value!.month.toString().padLeft(2,'0')}-${value!.day.toString().padLeft(2,'0')}';
    return InkWell(
      onTap: () async {
